...
const NavMenu: React.FC<Props> = ({
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    const focusable = Array.from(
      listRef.current.querySelectorAll<HTMLButtonElement>('.nav-item-button:not([disabled])'),
    )
    ...
    if (e.key === 'Enter' || e.key === ' ') {
      e.preventDefault()
      const btn = focusable[idx]
      btn?.click()
    }
  }

  return (
    <nav className={['nav-menu', orientation, className].filter(Boolean).join(' ')} aria-label={ariaLabel}>
      <ul
        className="nav-list"
        ref={listRef}
        role="menubar"
      ...
      >
        {items.map((item) => {
          ...
          return (
            <li key={item.key} className={`nav-item ${isActive ? 'active' : ''}`} role="none"> 
              {renderItem ? (
                <button
                  type="button"
                  role="menuitem"
